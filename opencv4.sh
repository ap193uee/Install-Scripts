...
apt-get -y install build-essential cmake pkg-config unzip git wget \
           gcc-6 g++-6 libgtk-3-dev 

echo "-----Installing Python libraries----"
apt-get -y install python-dev python-numpy

echo "-----Installing Image libraries----"
apt-get -y install libtiff-dev libpng-dev libjpeg-dev zlib1g-dev libwebp-dev
...
then    
    ...
    apt-get -y install libavcodec-dev libavformat-dev libswscale-dev libfaac-dev libmp3lame-dev libtheora-dev \
...
then
    ...
    apt-get -y install libgoogle-glog-dev libgphoto2-dev libgflags-dev libhdf5-serial-dev
